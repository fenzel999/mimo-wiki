...
## 搜索能力（Obsidian MCP + Omnisearch）

wiki 通过 Obsidian MCP Server 提供搜索能力，底层使用 [Omnisearch](https://github.com/scambier/obsidian-omnisearch) 插件的 BM25 引擎。

### 三种搜索模式

通过 MCP 工具 `obsidian_search_notes` 调用，`mode` 参数选择：

| mode | 方式 | 适用场景 | 说明 |
|------|------|---------|------|
| `text` | 子串匹配 | 快速查找关键词 | 最快，支持 `contextLength` 控制上下文 |
| `jsonlogic` | 结构化查询 | 按 frontmatter/tags/路径过滤 | 支持 glob、正则、数值比较 |
| `omnisearch` | BM25 排名 | 语义相关搜索 | 支持引号短语、`-排除`、`path:`/`ext:` 过滤、拼写容错 |

### 何时用哪种

- **定向/快速定位** → `text` 模式，搜关键词
- **按类型/标签/日期筛选** → `jsonlogic` 模式
- **"这个概念在哪出现过"** → `omnisearch` 模式（BM25）

### 小型 wiki（< 100 页面）

不需要 MCP 搜索——直接用 `read_file` 读 `index.md` 找相关页面，或用 `search_files` 搜关键词。
...
④ **大型 wiki（100+ 页面）** 用 MCP 搜索当前主题：
   - `obsidian_search_notes` mode=`omnisearch` 搜 BM25 相关页面
   - 或 `obsidian_search_notes` mode=`text` 搜关键词
...
① **捕获原始来源：**
   - URL → 用网络工具获取 markdown，保存到 `raw/articles/`
     推荐：Obsidian Web Clipper 浏览器扩展可快速将网页转 markdown
   - PDF → 保存到 `raw/papers/`
   - 粘贴文本 → 保存到适当的 `raw/` 子目录
...
① **读 `index.md`** 识别相关页面。
② **大型 wiki（100+ 页面）** 用 MCP 搜索：
   - `obsidian_search_notes` mode=`omnisearch` BM25 搜索
   - 或 mode=`text` 关键词搜索
③ **小型 wiki** 直接读 index.md 或 `search_files` 搜关键词。
④ **读相关页面**。
