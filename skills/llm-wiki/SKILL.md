---
name: llm-wiki
description: "Karpathy LLM Wiki: 构建和查询互链 markdown 知识库。包含完整模板、摄入/查询/检查步骤、Obsidian 集成和常见陷阱。当用户要求创建 wiki、摄入来源、查询 wiki、检查 wiki 健康度、或提到 wiki/知识库/笔记时使用。"
---
...
Wiki 就是一个 markdown 文件目录 — 可以在 Obsidian、VS Code 或任何编辑器中打开。
不需要数据库，不需要特殊工具。
...
```
# 按内容查找页面
搜索 "transformer" 在所有 .md 文件中

# 按文件名查找页面
查找 *.md 文件

# 按标签查找页面
搜索 "tags:.*alignment" 在 .md 文件中
...
当你需要代理在循环中策展时使用此技能；当你想要批量编译来源目录时使用 llm-wiki-compiler。
