...
关于 qmd 的安装、配置和详细用法，见 README.md 的"可选工具：qmd 搜索引擎"章节。
...
⑥ **更新 qmd 索引**（如果已配置）：`qmd update && qmd embed`
...
① **读 `index.md`** 识别相关页面。
② **如果 qmd 已配置**，用 `qmd query "问题" -c wiki` 搜索 — 混合 BM25 + 向量 + LLM 重排。
